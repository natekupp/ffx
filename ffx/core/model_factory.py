...
from .models import ConstantModel, ElasticNetWithTimeout, FFXModel
from .utils import nmse, nondominated_indices_2d, y_is_poor
...
class MultiFFXModelFactory:
    def build(self, train_X, train_y, test_X, test_y, varnames=None, verbose=False):
        ...
        if isinstance(train_X, np.ndarray) and varnames is None:
            raise Exception('varnames required for np.ndarray')
        ...
        for (i, approach) in enumerate(approaches):
            ...
            next_models = self._nondominated_models(next_models)
            models += next_models
            if verbose:
                print(
                ...
                for model in next_models:
                    print(
                        ...
                        % (model.num_bases, model.test_nmse, model.str2(500))
                    )

        # final pareto filter
        models = self._nondominated_models(models)
        ...
        if verbose:
            ...
            for (i, model) in enumerate(models):
                print(
                    ...
                    % (i + 1, len(models), model.num_bases, model.test_nmse, model.str2(500))
                )

        return models

    def _nondominated_models(self, models):
        test_nmses = [model.test_nmse for model in models]
        num_bases = [model.num_bases for model in models]
        I = nondominated_indices_2d(test_nmses, num_bases)
        models = [models[i] for i in I]

        I = np.argsort([model.num_bases for model in models])
        models = [models[i] for i in I]
...
class FFXModelFactory:
    def build(self, X, y, ss, varnames=None, verbose=False):
        ...
        if isinstance(X, pd.DataFrame):
            varnames = X.columns
            X = X.as_matrix()
        if isinstance(X, np.ndarray) and varnames is None:
            raise Exception('varnames required for np.ndarray')
            ...
            for exponent in ss.exprExponents():
                ...
                if exponent in [1.0, 2.0] or not y_is_poor(lin_yhat):
                    ...
                    for nonlin_op in ss.nonlin_ops():
                        ...
                        if np.all(nonlin_yhat == lin_yhat):
        ...
        if ss.include_interactions():
            ...
            order1_infls = np.abs(list(model.coefs_n[1:]) + list(model.coefs_d))  # influences
            I = np.argsort(-1 * order1_infls)
            ...
            for i, basei in enumerate(order1_bases):
                if not isinstance(basei, SimpleBase):
                    continue  # just xi
        ...
        var2_models = self._bases_to_models(
        ...
        # add constant; done
        models = [ConstantModel(np.mean(y), X.shape[0])] + models
        return models

    def _bases_to_models(
        self, ss, varnames, bases, X, y, max_num_bases, target_train_nmse, verbose
    ):
        # compute regress_X
        if ss.includeDenominator():
            regress_X = np.zeros((self.nrow, len(bases) * 2), dtype=float)
        else:
            regress_X = np.zeros((self.nrow, len(bases)), dtype=float)
        ...
        # compute models.
        models = self._pathwise_learn(
            ss=ss,
            varnames=varnames,
            bases=bases,
            X_orig_regress=regress_X,
            y_orig=y,
            max_num_bases=max_num_bases,
            target_nmse=target_train_nmse,
            verbose=verbose,
        )
        return models
    ...
    def _pathwise_learn(
        ...
        **fit_params
        ...
        for (alpha_i, alpha) in enumerate(alphas):
            ...
            # update nmses
            nmse_unbiased = nmse(
                np.dot(cur_unbiased_coefs, X_unbiased.T),
                y_unbiased,
                min(y_unbiased),
                max(y_unbiased),
            )
            ...
            # log
            num_bases = len(np.nonzero(cur_unbiased_coefs)[0])
    ...
    def _unbiasedXy(self, Xin, yin):
        ...
        # for y below.
        np.copyto(X_stds, 1.0, where=~(X_stds > 0.0))
        ...
        assert np.all(np.isfinite(X_unbiased))
        assert np.all(np.isfinite(y_unbiased))
    ...
    def _rebiasCoefs(self, unbiased_coefs, X_stds, X_avgs, y_std, y_avg):
        ...
        # main work
        n = len(X_stds)
        coefs = np.zeros(n + 1, dtype=float)
